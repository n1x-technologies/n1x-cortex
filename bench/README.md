...
and four hand-written subdirectories of `content/en/docs/reference`
(`glossary`, `networking`, `node`, `scheduling`): **584 documents, 962,044
tokens** measured with this bench's own tokenizer, CC BY 4.0. A haystack ~49×
the fixture, so finding the right note has to be earned, while staying small
enough that `full-context` remains runnable — above roughly 1M tokens it stops
being a baseline and becomes impossible, which would remove the quality ceiling
the whole comparison is measured against. That ceiling is what bounds the slice;
it leaves ~38k tokens of headroom.

**Most of `reference` is still excluded, but the exclusion used to be total and
that was too coarse.** Measured at the pinned commit, `reference` is 2.78M
tokens and 81% of it is the autogenerated material the exclusion was aimed at —
`kubernetes-api` 1.52M, `kubectl` 416k, `config-api` 179k, `setup-tools` 132k.
Generated docs have a structural regularity that flatters any retriever, so
those stay out.

The rest is hand-written prose that the other slices actively depend on: **218
of their files link into `reference`, 812 times.** Excluding all of it left
holes shaped exactly like the questions a benchmark most wants to ask, and two
were hit while writing question sets — RBAC had no definition anywhere in the
corpus, and `service.md` delegates traffic-policy mechanics to
`reference/networking`. `reference/glossary` matters most for its size: 27k
tokens that are the source every `{{< glossary_definition >}}` shortcode pulls
from.

Two known holes remain, excluded on size rather than principle:
`access-authn-authz` (112k, where RBAC lives) and `labels-annotations-taints`
(29k) would push the corpus to or past the 1M ceiling. **RBAC is therefore a
documented hole, not a surprise one** — do not write a question whose only
answer is there.

The script pins the upstream commit — genuinely, now: it checks out
`PINNED_COMMIT` and fails if it lands anywhere else. It previously defaulted to
`main` and merely *recorded* the resulting sha, so a re-run produced different
documents under the same instructions. It writes a `PROVENANCE.md` beside the
...
`claude-opus-5` judging. 20 asked, 20 scored, no errors.

It ran against the **pre-refinement slice** (`concepts` + `tasks`, 399 files),
before the `reference` subdirectories were added. That does not weaken the
number: `closed-book` is given no corpus at all, so contamination is a property
of the questions and the model, not of the slice. It does mean the questions'
`goldPaths` were chosen against the smaller corpus, and that a definitional
question like `p12` now has a better gold document than the one it names.
...
### The glossary shortcode, and why it changed the slice

24 files open with a `{{< glossary_definition >}}` shortcode instead of the
definition itself — Hugo injects that text at build time. Under the original
`concepts` + `tasks` slice the glossary was outside the corpus, so for those
topics the canonical one-line definition was **not in the corpus at all**:
`concepts/configuration/configmap.md` never said what a ConfigMap is. That is
what prompted including `reference/glossary`, and it is now fixed —
`reference/glossary/configmap.md` supplies "an API object used to store
non-confidential data in key-value pairs".

It is worth keeping in mind for a different reason now. The definition lives in
a **different document** from the concept page on the same topic, so a "what is
X" question is answerable but its answer is not where a reader would first look.
That is a legitimate retrieval problem rather than a corpus hole — which is the
point — but a question set should not treat the concept page as the only
acceptable gold path for a definitional question.
