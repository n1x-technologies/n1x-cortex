...
| `cortex embed` | Construye el almacén de embeddings (habilita la búsqueda semántica). En el dispositivo por defecto; con `--base-url <url>` embebe a través de un endpoint compatible con OpenAI, sin descargar ningún modelo ni necesitar el peer opcional. |
