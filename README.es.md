...
- **Medido, no afirmado.** Una suite de benchmarks reproducible compara Cortex contra baselines diseñados para ser difíciles de superar (contexto completo, un agente basado en grep, un RAG de chunks fijos con costo equiparado), usando un tokenizador BPE real en vez de una estimación por conteo de caracteres. Ambas etapas ya se ejecutan de principio a fin: el costo de recuperación sin modelo, y la calidad de las respuestas juzgada contra un modelo real, con un juez validado en 94,7 % de coincidencia con una persona que etiquetó a mano. Todavía reporta sobre un fixture de CI pequeño, que se satura: ahí todos los sistemas obtienen el mismo puntaje, así que no compara nada y no se deriva ninguna afirmación entre sistemas. El corpus público que sí podría hacerlo ya se puede descargar, pero su conjunto de preguntas aún no está escrito: un piloto de 20 preguntas midió que 19 de 20 ya se respondían sin contexto alguno, así que ese lote se retiró en vez de ampliarse y se está rehaciendo la estrategia de origen de las preguntas. Ver [`bench/README.md`](bench/README.md) para el método, los números actuales, y exactamente qué muestran y qué no.
...
| `cortex embed` | Construye el almacén de embeddings (habilita la búsqueda semántica). En el dispositivo por defecto; con `--base-url <url>` embebe a través de un endpoint compatible con OpenAI, sin descargar ningún modelo ni necesitar el peer opcional. Cada nota se corta a 16000 caracteres antes de enviarse, para que una nota larga entre en el contexto del endpoint. |
