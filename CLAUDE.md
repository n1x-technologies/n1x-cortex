...
| `bench/` | Reproducible measurements behind the README's "The numbers" section: `token-reduction.mjs` (per-query token savings on any vault) and `grounding-eval.mjs` (closed-book vs Cortex-grounded fabrication rate, via a local ollama endpoint). |
| `CLAUDE.md` | This file. |
