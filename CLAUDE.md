...
- **To work on the product** → `toolkit/` and `docs/superpowers/` (design specs + implementation plans).
...
| `toolkit/` | **The Cortex engine + agent** (Node/TS): reads any markdown vault into a note graph; CLI `init`/`status`/`orphans`/`viz`/`query`/`atomize`/`promote`/`set-status`/`undo`/`hook`/`pause`/`resume` · `gaps` · `dupes` · `merge` · `verify` · `moc` · `doc` · `embed`. `atomize` is **dry-run by default**: it creates `status: draft` notes in `_inbox/`, merges AI-distilled updates into existing notes in place (3.2), and (3.3) `promote` graduates status-advanced drafts out of `_inbox/` into curated folders. Every write is reversible (`.cortex/backups/` + `.cortex/promotions/`, `cortex undo`); `Markdown/` sources are never modified. AI distillation runs through `toolkit/skills/atomize/` (the `/atomize` skill). Phase 4 adds Claude Code lifecycle hooks (SessionStart reindex+status, Stop /atomize suggestion) that never auto-write — autonomy is detect-and-suggest, gated by `cortex pause`/`autonomy: off`. Phase 5 adds the curation layer — read-only diagnostics (gaps/dupes/verify) and producers (moc writes a reversible Map-of-Content note; doc consolidates a topic's notes into a branded Typst PDF via templates/typst/). Phase 6 adds the semantic layer — `cortex embed` builds a local, on-device embedding store (transformers.js) under `.cortex/embeddings/`, and `query`/`dupes` run hybrid lexical+semantic retrieval (RRF) that degrades to TF-IDF when the store is absent. No vault content leaves the machine. **Phase 7 makes autonomy real:** with `autonomy: auto-draft` or `full`, the `Stop` hook spawns a **headless `/atomize` run in the background** (`claude -p`) over the changed sources — `auto-draft` writes drafts to `_inbox/`, `full` also promotes the confident ones — so the brain captures sources without taking the user's turn. Guarded by an anti-recursion env flag (`CORTEX_AUTONOMY_CHILD`), a single-flight lock, a cooldown, and a per-session cap; every write is reversible (`cortex undo`) and `cortex pause`/`autonomy: off` is the kill switch. Phase 7 also closes the dedupe loop — `cortex merge` + the `/dupes-merge` skill reversibly merge a near-duplicate pair (redirecting inbound wikilinks) — and makes cited retrieval machine-consumable via `query --json` + the `/query` skill. Phases 0–7. **Published as `@n1x-technologies/cortex` on npm** (`npm i -g @n1x-technologies/cortex`; binary: `cortex`). `@xenova/transformers` is an **optional peer** — the base install is light; the semantic layer activates only when this peer is present. Releases are cut by pushing a `vX.Y.Z` git tag, which triggers the `release.yml` workflow to publish via `NPM_TOKEN`. |
| `docs/superpowers/` | Design spec (`specs/`) and implementation plans (`plans/`) for the toolkit. |
| `templates/` | Reusable N1X standards: `typst/` (branded document/PDF template), `readme/` (README standard), `collaboration/` (branch→PR→review workflow). Generic — any project can adopt them. |
| `sync/` | **Cross-project template sync** (`manifest` + `cortex-sync.sh`): Cortex is the source of truth for the shared templates; consumer repos pull updates with one command (engine files overwritten, instance files only flagged). |
| `CLAUDE.md` | This file. |
