...
4. **Releases:** the package is published by pushing a `vX.Y.Z` git tag, which triggers `.github/workflows/release.yml` (test → build → publish via the `NPM_TOKEN` secret → create a GitHub Release with auto-generated notes). Bump `toolkit/package.json` version in the PR that precedes the tag.
